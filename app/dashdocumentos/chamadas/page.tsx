...
import {
    Phone, Plus, Search, Pencil, Trash2,
...
/* ─────────────────────────────────────────────────────────────────────────────
   HELPERS
───────────────────────────────────────────────────────────────────────────── */
const emptyForm = (): Omit<Chamada, "id"> => ({
...
function FormBody({
    ...
    data: Omit<Chamada, "id"> & { id?: number };
    setData: (v: any) => void;
}) {
    return (
        <div className="px-6 py-5 space-y-5 overflow-y-auto max-h-[68vh]">
            ...
            <ModalSection title="Contacto" icon={User}>
                <div>
                    ...
                    <Input placeholder="Nome do chamador…" value={data.nome}
            ...
            <ModalSection title="Detalhes" icon={ClipboardList}>
                <div>
                    ...
                    <textarea rows={3} placeholder="Motivo da chamada…"
...
export default function Chamadas() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                console.error("Error fetching chamadas:", error);
            } finally {
                setLoading(false);
            }
        }
    ...
    const [viewItem, setViewItem] = useState<Chamada | null>(null);
    const [editItem, setEditItem] = useState<Chamada | null>(null);
    ...
    return (
        <>
            <Header />
            <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-50/30 pb-20">
                {/* ── Hero ── */}
                <div className="relative overflow-hidden bg-gradient-to-br from-orange-500 via-orange-500 to-amber-400 px-6 pt-10 pb-24">
                    ...
                    <div className="relative max-w-5xl mx-auto">
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/15 backdrop-blur-sm mb-4">
                                    ...
                                    <span className="text-xs font-bold text-white/80 uppercase tracking-widest">Registo de chamadas</span>
                                </div>
                                <h1 className="text-3xl md:text-4xl font-extrabold text-white leading-tight">Chamadas</h1>
                                <p className="text-blue-100/70 text-sm mt-1.5">Gestão e histórico de chamadas recebidas</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-8">
                            {[
                                { label: "Total de chamadas", value: records.length, icon: Phone },
                ...
                <div className="max-w-5xl mx-auto px-4">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-3">
                            <div className="w-10 h-10 rounded-full border-4 border-orange-200 border-t-blue-500 animate-spin" />
                            <p className="text-sm text-gray-400 font-medium">A carregar chamadas…</p>
                        </div>
                    ) : grouped.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-4">
                            ...
                            <div className="text-center">
                                <p className="font-bold text-gray-700 mb-1">Nenhuma chamada encontrada</p>
                                <p className="text-sm text-gray-400">
                                    {searchQuery ? "Tente outra pesquisa." : "Clique em \"Nova chamada\" para começar."}
                                </p>
                            </div>
                        </div>
                    ) : (
                                ...
                                {grouped.map(([monthKey, items]) => (
                                    <div key={monthKey}>
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <span className="text-xs font-bold text-gray-300">{items.length} chamada{items.length !== 1 ? "s" : ""}</span>
                                        </div>
                ...
                <DialogContent className="rounded-3xl max-w-lg p-0 overflow-hidden border-0 shadow-2xl shadow-black/20 gap-0">
                    <div className="bg-gradient-to-br from-orange-500 to-amber-400 px-6 py-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <DialogTitle className="text-white text-lg font-extrabold">Nova Chamada</DialogTitle>
                                <p className="text-white/60 text-xs mt-0.5">Preencha os dados da chamada</p>
                            </div>
                        </div>
                    </div>
                ...
                <DialogContent className="rounded-3xl max-w-lg p-0 overflow-hidden border-0 shadow-2xl shadow-black/20 gap-0">
                    <div className="bg-gradient-to-br from-orange-500 to-amber-400 px-6 py-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <DialogTitle className="text-white text-lg font-extrabold">Editar Chamada</DialogTitle>
            ...
            <Dialog open={deleteConfirmId !== null} onOpenChange={open => { if (!open) setDeleteConfirmId(null); }}>
                <DialogContent className="rounded-3xl max-w-sm p-0 overflow-hidden border-0 shadow-2xl shadow-black/20 gap-0">
                    <div className="p-8 text-center">
                        ...
                        <DialogTitle className="text-gray-900 text-lg font-extrabold mb-2">Eliminar chamada?</DialogTitle>
